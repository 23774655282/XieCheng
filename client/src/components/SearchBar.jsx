...
import { formatLocalDate, formatDateShort, formatDateCompact, formatDateSuffix, getTodayLocal, parseLocalDate } from '../utils/dateUtils';
...
function SearchBar({ compact = false, datePickerOpen: datePickerOpenProp, setDatePickerOpen: setDatePickerOpenProp, onOpenDatePicker, heroStyle = false }) {
  ...
  const performSearch = useCallback((overrides = {}) => {
    ...
    const cIn = overrides.checkIn ?? checkIn;
    const cOut = overrides.checkOut ?? checkOut;
    ...
    params.set('destination', String(dest).trim());
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e) => {
      if (destinationRef.current?.contains(e.target)) return;
      if (e.target.closest('[data-destination-dropdown]')) return;
  ...
  const formClass = compact
    ? 'search-bar-form w-full h-full min-h-12 bg-white rounded-xl text-gray-500 px-2 flex flex-row flex-wrap items-center gap-0 shadow-sm border border-gray-200'
    : `search-bar-form w-full bg-white/80 backdrop-blur-sm text-gray-500 rounded-lg px-6 py-5 flex flex-col md:flex-row md:items-end max-md:items-start max-md:mx-auto w-full ${heroStyle ? 'gap-3 shadow-lg' : 'gap-4 shadow-sm border border-gray-100'}`;
      ...
      <div className="w-full h-full min-h-0" onClick={(e) => { if (!e.target.closest('input, button, a, label, [role="option"], [role="listbox"], ul, li, .react-datepicker-wrapper')) document.activeElement?.blur?.(); }} onFocus={(e) => { const t = e.target; if (t.tagName === 'INPUT' && t.id !== 'searchBar-destination') setTimeout(() => t.blur(), 0); }}>
        <form onSubmit={handleSubmit} className={formClass}>
          ...
          <div className="flex-1 min-w-0 flex flex-col items-center justify-center overflow-visible py-1 px-1 relative">
            <button type="button" onClick={() => { setDestinationOpen(false); setGuestsOpen(false); onOpenDatePicker?.(); setDatePickerOpen((o) => !o); }} className={`text-sm font-medium py-0.5 text-center block w-full min-w-0 leading-tight cursor-pointer ${datePickerOpen ? 'text-gray-700' : 'text-gray-900'}`}>
              <span className="block truncate">住 {checkIn ? formatDateCompact(parseLocalDate(checkIn)) : '—'}</span>
                ...
                <div className="fixed left-0 right-0 w-full bg-white border-t border-gray-200 shadow-lg flex flex-col" style={{ top: '10rem', bottom: 0, maxHeight: 'calc(100vh - 10rem)' }} onClick={(e) => e.stopPropagation()}>
                  <div className="flex-1 min-h-0 overflow-y-auto p-4 compact-datepicker-inline">
                    <DatePicker
                      ...
                      onChange={(dates) => { let [start, end] = dates || [null, null]; if (start && end && end <= start) end = addDays(start, 1); setCheckIn(start ? formatLocalDate(start) : ''); setCheckOut(end ? formatLocalDate(end) : ''); }}
                      minDate={new Date()}
                      monthsShown={12}
                      locale={zhCN}
                      calendarStartDay={0}
                  ...
                  <div className="flex-shrink-0 p-4 border-t border-gray-100">
                    <button type="button" onClick={() => { setDatePickerOpen(false); performSearch(); }} className="w-full py-2.5 rounded-lg bg-gray-900 text-white text-sm font-medium">确定</button>
                  </div>
                </div>
              </div>,
              document.body
          ...
          <span className="w-px h-4 bg-gray-300 shrink-0" aria-hidden />
          <div className="relative flex-1 min-w-0 flex flex-col items-center justify-center overflow-visible py-1 px-1" ref={guestsRef}>
            <button type="button" onClick={() => { setDatePickerOpen(false); setGuestsOpen((o) => !o); }} className="text-sm font-medium text-gray-900 py-0.5 cursor-pointer text-center block w-full min-w-0 leading-tight">
              <span className="block truncate">{rooms}间</span>
              <span className="block truncate text-gray-500">{adults + children}人</span>
            </button>
            {guestsOpen && (
              <div className="absolute top-full right-0 left-auto mt-1 py-3 px-4 rounded-lg bg-white border border-gray-200 shadow-lg z-50 min-w-[200px]">
          ...
          <button type="submit" className="shrink-0 p-2 rounded-lg bg-gray-900 text-white">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="m21 21-3.5-3.5M17 10a7 7 0 1 1-14 0 7 7 0 0 1 14 0Z" /></svg>
          </button>
        </form>
      </div>
    );
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className={formClass}>
      ...
      <div className={`relative flex md:flex-col max-md:gap-2 max-md:items-center shrink-0 w-full ${heroStyle ? 'md:flex-shrink-0 md:w-[240px]' : 'md:min-w-[260px]'}`} ref={guestsRef}>
        <div className="flex items-center gap-2 shrink-0"><img src={assets.guestsIcon} alt="" className="w-4 h-4 flex-shrink-0 opacity-70" /><label className="mb-1.5 md:mb-0 whitespace-nowrap">人数</label></div>
        ...
        {guestsOpen && (
          <div className="absolute top-full left-0 right-0 mt-2 py-4 px-4 rounded-lg bg-white/95 backdrop-blur-sm border border-gray-200/80 shadow-lg z-20 w-full md:min-w-[220px]">
            ...
            <button type="button" onClick={() => { setGuestsOpen(false); performSearch(); }} className="mt-4 w-full py-2.5 rounded-lg bg-black text-white text-sm font-medium hover:bg-gray-800">完成</button>
          </div>
        )}
        </div>
      </div>

      <div className={`flex flex-row gap-2 shrink-0 ${heroStyle ? 'w-full md:flex-shrink-0 md:w-[120px]' : 'max-md:w-full max-md:flex-1'}`}>
        <button type="submit" className={`flex items-center justify-center gap-1 rounded-md bg-black py-2.5 px-4 text-white text-sm cursor-pointer min-w-0 min-h-[56px] ${heroStyle ? 'flex-1 md:w-full' : 'flex-1'}`}>

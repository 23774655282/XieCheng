...
function SearchBar() {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="search-bar-form w-full bg-white/80 backdrop-blur-sm text-gray-500 rounded-lg px-6 py-5 flex flex-col md:flex-row md:items-end max-md:items-start gap-4 max-md:mx-auto shadow-sm border border-gray-100">
      <div className="relative min-w-0 max-w-[200px] md:max-w-[220px]" ref={destinationRef}>
        <div className="flex items-center gap-2"><SlCalender /><label htmlFor="searchBar-destination">目的地</label></div>
        <input onChange={(e) => { setDestination(e.target.value); setDestinationOpen(true); setDestinationError(''); }} onFocus={() => setDestinationOpen(true)} value={destination} id="searchBar-destination" type="text" className="rounded-lg border border-gray-200 px-3 py-2 mt-2 text-sm font-semibold text-gray-900 placeholder:font-normal placeholder:text-gray-400 outline-none w-full min-w-0 min-h-[56px]" placeholder="输入或选择目的地" autoComplete="off" />
      ...
      <div className="min-w-[380px] flex-1">
        ...
        <DatePicker id="searchBar-checkInOut" selectsRange startDate={checkIn ? parseLocalDate(checkIn) : null} endDate={checkOut ? parseLocalDate(checkOut) : null} onChange={(dates) => { let [start, end] = dates || [null, null]; if (start && end && end <= start) end = addDays(start, 1); setCheckIn(start ? formatLocalDate(start) : ''); setCheckOut(end ? formatLocalDate(end) : ''); }} minDate={new Date()} monthsShown={2} customInput={<button type="button" className="rounded-lg border border-gray-200 px-0 py-2 mt-2 text-sm outline-none w-full min-w-0 cursor-pointer caret-transparent select-none text-left min-h-[56px] flex items-stretch overflow-hidden"><div className="flex-1 flex flex-col justify-center px-4 text-left"><div className="font-semibold text-gray-900">{checkIn ? formatDateShort(parseLocalDate(checkIn)) : '—'}{checkIn && <span className="text-xs font-normal text-gray-500 ml-1">{formatDateSuffix(parseLocalDate(checkIn))}</span>}</div></div><div className="flex items-center justify-center gap-2 px-4 text-gray-500 text-sm shrink-0 cursor-default" onClick={(e) => e.stopPropagation()}><span className="text-gray-400">—</span><span>{checkIn && checkOut ? differenceInCalendarDays(parseLocalDate(checkOut), parseLocalDate(checkIn)) : 0}晚</span><span className="text-gray-400">—</span></div><div className="flex-1 flex flex-col justify-center px-4 text-right"><div className="font-semibold text-gray-900">{checkOut ? formatDateShort(parseLocalDate(checkOut)) : '—'}{checkOut && <span className="text-xs font-normal text-gray-500 ml-1">{formatDateSuffix(parseLocalDate(checkOut))}</span>}</div></div></button>} popperClassName="hero-datepicker-popper" locale={zhCN} calendarStartDay={0} onKeyDown={(e) => { if (e.key !== 'Tab') e.preventDefault(); }} />
      </div>

      <div className="relative flex md:flex-col max-md:gap-2 max-md:items-center shrink-0 min-w-[230px] md:min-w-[260px]" ref={guestsRef}>
